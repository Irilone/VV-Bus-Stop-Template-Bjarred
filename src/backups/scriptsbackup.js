 ...
 * @author Claude Code
 * @version 3.2 - Shadow DOM, block-upgrade, and CMS-safe scoping
 */
...
document.addEventListener('DOMContentLoaded', ()=> {
   ...
   */
        ...
        document.querySelectorAll('img[data-fluid]').forEach(img=> {
            ...
            ?width=1200&mode=max&format=auto&q=80`;
   ...
   */
    ...
    if ('IntersectionObserver' in window) {
        const fadeObserver=new window.IntersectionObserver((entries, observerInstance)=> {
...
const footerLinkFixes= {
    'Tillgänglighet': 'https://www.praktikertjanst.se/mer/om-oss/tillganglighet/tillganglighet-for-www.ptj.se/',
    'Synpunkter på vården': 'https://www.praktikertjanst.se/praktisk-information/synpunkter-pa-varden/'
}
...
function initGDPRCompliance() {
    ...
    if ( !hasConsent && !document.querySelector('.gdpr-banner')) {
        ...
        banner.innerHTML=` <div style="max-width: 1200px; margin: 0 auto; display: flex; align-items: center; gap: var(--space-md); flex-wrap: wrap;" > <p style="flex: 1; margin: 0; font-size: var(--fs-sm);" > Vi använder endast tekniskt nödvändiga cookies för att säkerställa att vår tjänst fungerar korrekt. Inga personuppgifter lagras utan ditt samtycke enligt GDPR. </p> <div style="display: flex; gap: var(--space-sm);" > <button id="gdpr-accept" style="background: white; color: var(--clr-primary); border: none; padding: var(--space-xs) var(--space-md); border-radius: var(--radius-sm); font-weight: 600; cursor: pointer;" > Jag förstår </button> <button id="gdpr-info" style="background: transparent; color: white; border: 1px solid white; padding: var(--space-xs) var(--space-md); border-radius: var(--radius-sm); cursor: pointer;" > Mer info </button> </div> </div> `;
...
function initCSSVariableIntegration() {
    ...
    dynamicElements.forEach(el=> {
            ...
            switch(styleType) {
                case 'card': el.style.transition=`all $ {
                    cssVars.durationNormal
                }
                ...
                case 'hover': el.style.transition=`transform $ {
                    cssVars.durationFast
                }
...
function initPWAFeatures() {
    ...
    window.addEventListener('beforeinstallprompt', (e)=> {
            ...
            installBanner.innerHTML=` <div style="background: var(--clr-bg-alt); padding: var(--space-sm); border-radius: var(--radius-md); margin: var(--space-md) 0; display: flex; align-items: center; gap: var(--space-md);" > <span>📱</span> <div style="flex: 1;" > <strong>Installera Vaccinbussen-appen</strong> <br><small>Få snabb tillgång till vaccinationsschema och priser</small> </div> <button id="install-pwa" style="background: var(--clr-primary); color: white; border: none; padding: var(--space-xs) var(--space-md); border-radius: var(--radius-sm); cursor: pointer;" > Installera </button> <button id="dismiss-pwa" style="background: transparent; border: none; font-size: 1.2em; cursor: pointer;" >×</button> </div> `;
...
(function() {
        ...
        // ═══════════════════════════════════════════════════════════════
        // SEKTION X: CMS-SAFE SCOPING: Upgrade vaccine-bjarred-app blocks to Custom Element with Shadow DOM
        // ═══════════════════════════════════════════════════════════════

        class VaccineBjarredApp extends HTMLElement {
            constructor() {
                super();
        ...
        connectedCallback() {
            let content=this._originalContent || '';
            if ( !content && this.innerHTML) content=this.innerHTML;

            // Shadow DOM with style isolation
            this.shadowRoot.innerHTML=` <style> :host {
                display: block; font-family: 'Segoe UI', Arial, sans-serif; background: #f7fbfa; padding: 1.5rem; border-radius: 1rem; box-shadow: 0 2px 16px rgba(0, 0, 0, 0.07);
            }
            ...
            .location-schedule th, .location-schedule td {
            ...
            @media (max-width: 768px) {
                .location-schedule td, .location-schedule th {
                    font-size: 0.95em; padding: 6px 6px;
                }
            }
    ...
    if ( !customElements.get('vaccine-bjarred-app')) {
        customElements.define('vaccine-bjarred-app', VaccineBjarredApp);
    }

    // Upgrade all div.vaccine-bjarred-app blocks
    document.querySelectorAll('div.vaccine-bjarred-app').forEach(div=> {
            const el=document.createElement('vaccine-bjarred-app');
            el._originalContent=div.innerHTML;

            Array.from(div.attributes).forEach(attr=> {
                    if ( !['class'].includes(attr.name)) el.setAttribute(attr.name, attr.value);
                });
            div.replaceWith (el);
        });
});