...
export class EventManager extends EventManagerContract {
  constructor(options = {}) {
    ...
    this.init(options.configPath);
  }

  /**
   * Initialize and load event types
   * @param {string} configPath - Path to event types JSON
   */
  async init(configPath = '/globalassets/vaccincenter-varvet/scripts/atomic-components/design-tokens/event-types.json') {
    await this.loadEventTypes(configPath);
  }

  /**
   * Load event configuration from design tokens
   */
  async loadEventTypes(configPath) {
    try {
      // Add cache busting timestamp
      const separator = configPath.includes('?') ? '&' : '?';
      const cacheBustedPath = `${configPath}${separator}v=${Date.now()}`;
  ...
  resolveElement(element) {
    ...
    if (!(element instanceof Element) && element !== document && element !== window) {
